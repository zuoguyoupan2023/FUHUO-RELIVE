...
const fuhuoDir = path.join(rootDir, 'fuhuo');  // 🆕 Répertoire du protocole Relive
...
const buildEntries = async () => {
  ...
  const fuhuoFiles = await listFiles(fuhuoDir);  // 🆕 Scanner les fichiers du protocole Relive
  ...
  // Fichier de configuration - placer sous _config/ à la racine pour éviter la confusion avec openclaw/
...
const run = async () => {
  console.log('🚀 Démarrage du protocole FUHUO Upload...\n');
  ...
  console.log(`📊 Fichiers locaux: ${localMap.size}`);
  console.log(`📊 Fichiers distants: ${remoteMap.size}\n`);
  ...
  console.log(`📤 À uploader: ${uploadList.length} fichiers`);
  console.log(`🗑️  À supprimer: ${deleteList.length} fichiers\n`);
  ...
  if (deleteList.length > 0) {
    ...
    console.log(`  ✅ Supprimé ${deleteList.length} fichiers`);
  }
  ...
  console.log('\n' + '─'.repeat(60));
  console.log('✅ FUHUO Upload terminé !');
  console.log(`📦 Bucket: ${bucket}`);
  console.log(`📁 Préfixe: ${basePrefix || '(root)'}`);
