...
const fuhuoDir = path.join(rootDir, 'fuhuo');  // 🆕 Relive protocol directory
...
const buildEntries = async () => {
  ...
  const fuhuoFiles = await listFiles(fuhuoDir);  // 🆕 Scan Relive protocol files
  ...
  // Config file - place under root _config/ to avoid mixing with openclaw/
...
const run = async () => {
  console.log('🚀 Starting FUHUO Upload protocol...\n');
  ...
  console.log(`📊 Local files: ${localMap.size}`);
  console.log(`📊 Remote files: ${remoteMap.size}\n`);
  ...
  console.log(`📤 To upload: ${uploadList.length} files`);
  console.log(`🗑️  To delete: ${deleteList.length} files\n`);
  ...
  if (deleteList.length > 0) {
    ...
    console.log(`  ✅ Deleted ${deleteList.length} files`);
  }
  ...
  console.log(`📁 Prefix: ${basePrefix || '(root)'}`);
