...
const restoreFile = async (rel) => {
  ...
  if (rel.startsWith('_config/')) {
    // Map _config/ to /root/.openclaw or /root/.clawdbot
...
const run = async () => {
  console.log('🔄 Starting FUHUO Relive protocol...\n');
  ...
  console.log(`📋 Found ${relPaths.length} files to restore\n`);
  ...
  console.log(`📁 Restore location: ${rootDir}`);
