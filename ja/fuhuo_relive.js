...
const restoreFile = async (rel) => {
  ...
  if (rel.startsWith('_config/')) {
    // _config/ を /root/.openclaw または /root/.clawdbot に対応
...
const run = async () => {
  ...
  console.log(`📋 復元が必要なファイル: ${relPaths.length} 件\n`);
  ...
  console.log('\n' + '─'.repeat(60));
  console.log('✅ FUHUO 复活 完了!');
  console.log(`📦 バケット: ${bucket}`);
  console.log(`📁 復元先: ${rootDir}`);
