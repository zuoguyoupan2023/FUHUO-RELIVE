...
const restoreFile = async (rel) => {
  ...
  if (rel.startsWith('_config/')) {
    // Сопоставление _config/ с /root/.openclaw или /root/.clawdbot
...
const run = async () => {
  console.log('🔄 Начало протокола FUHUO Relive...\n');
  ...
  console.log(`📋 Найдено ${relPaths.length} файлов для восстановления\n`);
  ...
  console.log('\n' + '─'.repeat(60));
  console.log('✅ FUHUO Relive завершён!');
  console.log(`📦 Bucket: ${bucket}`);
  console.log(`📁 Путь восстановления: ${rootDir}`);
